									   ...
									   selTdata *ptSelData )
{
  ...
  unsigned	iObjSize;
	...
	* Also davon bitt nicht irritieren lassen.
...
#if 0
static void SetFocus2Elem(MskDialog mask, const std::string & ef_name)
{
  MskStatic  my_ef=NULL;
  MskElement my_ele=NULL;

  if (!mask || ef_name.empty() )
    return;

  /* Alle anderen Focus Aktionen löschen */
  WdgKillFocusEventQ();
  /* Element holen */
  my_ef=MskGetElement(TO_CHAR(ef_name));
  if (my_ef)
    {
      my_ele=MskQueryRl(mask, my_ef, KEY_DEF);
    }
  if (my_ele)
    {
      WdgSetFocusEventQ(my_ele->ec_rl.w);
    }
}
#endif
...
#if TOOLS_VERSION >= 38		
	    // TODO wie teile ich das dem Haubi mit?
	    // siehe Bugzilla http://bugzilla.salomon.at/show_bug.cgi?id=31435
		ListDialogSet(pListDialog, ListNviewCbInputReasonFilter, (void*)my_list_view_intput_reason_filter);
#endif		
	}
	...
	if( start_direct )
	  {
	    ...
	    pListXDialog->mask_rl = mask_rl;
            ...
            if( start_blocking )
              ListDialogSet(pListDialog, ListNisModalMask, (void*)1);
	...
	else
	  {
	    ...
	    std::cout << "mask_rl normal " << mask_rl << std::endl;
